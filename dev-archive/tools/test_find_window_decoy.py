...
`Alice: Madness Returns` owned by **python.exe**, and requires that:
...
DECOY_TITLE = "Alice: Madness Returns"          # exactly what find_window() looks for
WS_OVERLAPPEDWINDOW = 0x00CF0000
SW_SHOWNA = 8


def load_alice_harness():
    """Import alice_harness without running its __main__ block."""
    path = os.path.join(HERE, "alice_harness.py")
    spec = importlib.util.spec_from_file_location("alice_harness_under_test", path)
...
def make_decoy():
    """A real visible top-level window titled exactly like Alice's, owned by us."""
...
def main():
    ...
    try:
        ...
        try:
            hits = dd.find_window(require_process=False)
            if not hits:
                raise SystemExit('no window matched by title alone')
            h, t = hits[0]
            if h == hwnd:
