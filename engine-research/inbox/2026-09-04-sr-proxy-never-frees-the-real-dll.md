...
(`LoadLibraryA(sysdir)`, around line 97) and **contains no `FreeLibrary` call anywhere**
...
Wake, and this project has not yet reached the point where a renderer restart would be noticed. It is worth closing anyway, because when it does bite it costs a whole session to
